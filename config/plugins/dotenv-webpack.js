...
class Dotenv {
   ...
   */
  constructor({ path = './.env', safe, systemvars, silent, sample } = {}) {
    ...
    if (sample) {
      ...
      this.warn(
        'dotenv-webpack: "options.sample" is a deprecated property. Please update your configuration to use "options.safe" instead.',
        silent,
      );
    }
    ...
    const formatData = Object.keys(vars).reduce((obj, key) => {
      obj[`process.env.${key}`] = `(process.env.${key} ? process.env.${key} : ${JSON.stringify(vars[key])})`;
      return obj;
    }, {});
